        ...
        try:
            if self.enabled_jwt == "1":
                ...
                if 'pkgtype' in jwt_data and "webzip" in jwt_data['pkgtype']:
                    returnurl=jwt_data["returnurl"].split("/mod/exeweb")[0].split("/course/view.php")[0].split("/?redirect=0")[0].split("/course/section.php")[0]
                    returnurl=returnurl+"/mod/exeweb/set_ode.php"
                else:
                    returnurl=jwt_data["returnurl"].split("/mod/exescorm")[0].split("/course/view.php")[0].split("/?redirect=0")[0].split("/course/section.php")[0]
