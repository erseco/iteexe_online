    ...
    def getChild(self, name, request):
        ...
        if 'user' in request.args and request.args['user'][0]:
            if self.integration.enabled_jwt == "0":
                ...
                if 'jwt_token' in request.args and request.args['jwt_token'][0]:
                    multidomainuser=jwt.decode(request.args['jwt_token'][0],self.integration.jwt_secret_key, algorithms=self.integration.jwt_secret_hash)["returnurl"].split("/mod/exescorm")[0].split("/mod/exeweb")[0].split("/course/view.php")[0].split("/?redirect=0")[0].split("/course/section.php")[0]
